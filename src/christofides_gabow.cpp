...
int main(int argc, char* argv[]) {
    ...
    vector<int> path = christofidesPath_gabow(coords);
    ...
    // Save tour
    ofstream out("result/christofides_gabow_" + datasetName + ".tour");
    if (!out.is_open()) {
        cerr << "Failed to create output file" << endl;
        return 1;
    }
    out << "NAME : christofides_gabow_" << datasetName << "\nTYPE : TOUR\nDIMENSION : " << coords.size() << "\nTOUR_SECTION\n";
    for (int idx : path)
        out << (idx + 1) << "\n";
    out << "-1\n";
    out.close();
    ...
    printResults("Christofides (Gabow)", cost, elapsed, gtCost, gapCount);
