...
int main(int argc, char* argv[]) {
    ...
    vector<int> path = christofidesPath_edmonds(coords);
    auto end = chrono::high_resolution_clock::now();
    double elapsed = chrono::duration_cast<chrono::duration<double>>(end - start).count();

    if (path.empty()) {
        cerr << "Christofides Failed." << endl;
        return 1;
    }

    // Save tour
    ofstream out("result/christofides_edmonds_" + datasetName + ".tour");
    if (!out.is_open()) {
        cerr << "Failed to create output file" << endl;
        return 1;
    }
    out << "NAME : christofides_edmonds_" << datasetName << "\nTYPE : TOUR\nDIMENSION : " << coords.size() << "\nTOUR_SECTION\n";
    for (int idx : path)
        out << (idx + 1) << "\n";
    out << "-1\n";
    out.close();
    ...
    printResults("Christofides (Edmonds)", cost, elapsed, gtCost, gapCount);
