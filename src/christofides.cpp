...
int main(int argc, char* argv[]) {
    ...
    vector<int> path = christofidesPath(coords);
    ...
    // Save tour
    ofstream out("result/christofides_" + datasetName + ".tour");
    out << "NAME : christofides_" << datasetName << "\nTYPE : TOUR\nDIMENSION : " << coords.size() << "\nTOUR_SECTION\n";
    for (int idx : path)
        out << (idx + 1) << "\n";
    out << "-1\n";
    out.close();
    ...
    cout << "Christofides Tour cost: " << cost << endl;
